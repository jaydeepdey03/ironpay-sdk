...
export function JaydeepButton({ text, amount, address, ...props }: ButtonProps) {
  ...
  useEffect(() => {
    ...
    const getContract = () => {
      ...
      if ((window as any).ethereum) {
        ...
        const WIRON_Contract = new ethers.Contract(contractAddress, contractABI, signer);
  ...
  const transferWIRON = (amount: Number, address: string) => {
    ...
    if ((window as any).ethereum && contract) {
      contract.transferWithMetadata("0x664b8b9892b7560b356ef0f8d44cbd1f6628e388", amount, address);
    }
  }

  const { className, ...restProps } = props;
  return (
    <button className={`${className} my-button`} {...restProps} onClick={() => transferWIRON(amount, address)}>
      {text}
    </button>
  );
}
